...
mdmb_regression_optim_oprobit_grad <- function(x, index_beta, index_thresh, Xdes, offset_values,
    y, eps, h, weights, NT, use_rcpp_deriv_ypred, use_rcpp_deriv_logthresh )
{
    beta <- x[ index_beta ]
    ...
    if (NT>0){
        ...
        for (ii in 1:NT){        
            ...
            if ( use_rcpp_deriv_logthresh == 2 ){                    
                ...
                thresh_der <- c( mdmb_oprobit_extend_thresh( thresh =thresh_der, max_val=0 )$thresh_low, 0)
                der1 <- mdmb_rcpp_oprobit_derivative_logthresh_chain_rule( 
                                thresh_der=thresh_der, probs=probs, dens_upp=dens_upp, dens_low=dens_low, y=y, 
                                eps=eps, y_value=ii ) 
            }
